    ...
    async def get(self, username, server_name=''):
        ...
        if spawner.ready:
            ...
            try:
                ...
                if f and f.done() and f.exception():
                    failed_event['message'] = "Start failed: %s" % f.exception()
                    self.log.info("action=failure - Server %s didn't start", spawner._log_name)
                else:
                    self.log.warning(
                ...
                if spawner._error:
                    if spawner._error == 'cancelclick':
                        ...
                        failed_event['html_message'] = "Start failed: %s" % spawner._error
                    self.log.info("action=failure - Server %s didn't start: %s", spawner._log_name, spawner._error)
                else:
...
default_handlers = [
    ...
    (r"/api/users/([^/]+)/servers/([^/]*)/status/([^/]+)", SpawnProgressUpdateAPIHandler),
